...
for t in range(n_steps):
    ...
    # compute update
    w_update= grad_flat.mm(torch.inverse(Hessian)) 
    w_1_update=w_update[0][0:D_in*H1].view(D_in,H1)
    w_2_update=w_update[0][D_in*H1:d].view(H1,D_out)

    # Update weights using gradient descent;
    w1.data -= learning_rate * w_1_update
    w2.data -= learning_rate * w_2_update
...
np.save('loss_1HL_Newton',loss_collector)
np.save('grads_1HL_Newton',grad_collector)
np.save('EVs_1HL_Newton', EV_collector)
